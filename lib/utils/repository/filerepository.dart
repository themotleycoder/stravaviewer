      ...
      for (var obj in json) {
        ...
        if (route.eventOnly.toLowerCase() != 'run only' &&
          ...
          if (!routes.containsKey(route.id)) {
            routes[route.id] = new List<RouteData>();
          }
    ...
    if (response.statusCode == 200) {
      ...
      for (dynamic val in vals) {
        ...
        RouteData route = new RouteData(
            url, world, distance, altitude, eventOnly, routeName, id);
        if (route.eventOnly.toLowerCase() != 'run only' &&
            route.eventOnly.toLowerCase() != 'run only, event only') {
          if (!routes.containsKey(id)) {
            routes[id] = new List<RouteData>();
          }
          routes[id].add(route);
        }
      }
    ...
    try {
      ...
      json.forEach((key, worldRoute) {
        ...
        for (dynamic w in worldRoute) {
          list.add(new WorldData(w['id'], null, w['name'], w['url']));
        }
